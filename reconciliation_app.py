    ...
    if "password_correct" not in st.session_state or not st.session_state["password_correct"]:
        st.markdown("""
            <div class="login-card">
                <h2>🔒 Secure Login</h2>
                <p><b>Cash Tracker</b><br>Daily sales</p>
            </div>
        """, unsafe_allow_html=True)
...
with st.sidebar:
    ...
    app_mode = st.radio("Select View", ["Daily Data Entry & Audit", "REPORT", "MONTHLY REVIEW"])
...
if app_mode == "Daily Data Entry & Audit":
    st.markdown("<h1><b>Cash Tracker</b><br><span style='font-size: 20px; font-weight: normal;'>Daily sales</span></h1>", unsafe_allow_html=True)
...
# ----------------------------------------------------
# VIEW 2: MULTI-DATE RANGE REPORT (THE "FULL MONTY") TAB
# ----------------------------------------------------
elif app_mode == "REPORT":
    st.title("📅 Multi-Date Range Report ('Full Monty')")
    st.write("Select a custom date range to aggregate all cash, mobile money, expenses, and drawer totals between any two dates for client review and printing.")
    ...
    report_business_name = st.text_input("Business Name for Range Report", "BUSINESS CLIENT")
    ...
    if st.button("📊 Generate Multi-Date Range Report"):
        ...
        for d_str, data in st.session_state.daily_records.items():
            ...
            if start_date <= d_obj <= end_date:
                ...
                c_sales = sum([t["Amount"] for t in data["transactions"] if t["Type"] == "Cash"])
                m_sales = sum([t["Amount"] for t in data["transactions"] if t["Type"] == "Mobile Money"])
                cd_sales = sum([t["Amount"] for t in data["transactions"] if t["Type"] == "Card / Bank"])
                cr_sales = sum([t["Amount"] for t in data["transactions"] if t["Type"] == "Credit (Owed)"])
        ...
        total_agg_revenue = agg_cash_sales + agg_mobile_money + agg_card_sales + agg_credit_sales

        st.divider()

        range_report_html = f"""
        <div id="printable-report" style="width: 100%; max-width: 750px; margin: 0 auto; padding: 25px; border: 1px solid #ccc; border-radius: 8px; background-color: #ffffff; color: #000; font-family: Arial, sans-serif; font-size: 13px;">
            <h2 style="text-align: center; margin-bottom: 2px; font-size: 22px;"><b>Cash Tracker</b><br><span style="font-size: 15px; font-weight: normal;">Daily sales</span></h2>
            <p style="text-align: center; font-size: 14px; color: #555; margin-top: 0;">{report_business_name} - Comprehensive Range Financial Report ("Full Monty")</p>
            <p style="text-align: center; font-size: 12px; color: #777;">Period: {start_date} to {end_date}</p>
            <hr style="border: 0; border-top: 1px solid #ddd; margin: 15px 0;">
            
            <h3 style="font-size: 15px; margin-bottom: 5px;">1. Summary Totals Between Selected Dates</h3>
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 15px;">
                <tr>
                    <td style="padding: 6px; border-bottom: 1px solid #eee;"><b>Total Cash Collected:</b></td>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: right;">{agg_cash_sales:,.2f} TZS</td>
                </tr>
                <tr>
                    <td style="padding: 6px; border-bottom: 1px solid #eee;"><b>Total Mobile Money Collected:</b></td>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: right;">{agg_mobile_money:,.2f} TZS</td>
                </tr>
                <tr>
                    <td style="padding: 6px; border-bottom: 1px solid #eee;"><b>Total Card / Bank Sales:</b></td>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: right;">{agg_card_sales:,.2f} TZS</td>
                </tr>
                <tr>
                    <td style="padding: 6px; border-bottom: 1px solid #eee;"><b>Total Credit Sales:</b></td>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: right;">{agg_credit_sales:,.2f} TZS</td>
                </tr>
                <tr style="background-color: #f1f5f9;">
                    <td style="padding: 8px; font-weight: bold;">Combined Total Revenue:</td>
                    <td style="padding: 8px; font-weight: bold; text-align: right;">{total_agg_revenue:,.2f} TZS</td>
                </tr>
                <tr>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; color: #b91c1c;"><b>Total Expenses / Cash Paid Out:</b></td>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: right; color: #b91c1c;">{agg_expenses:,.2f} TZS</td>
                </tr>
                <tr>
                    <td style="padding: 6px; border-bottom: 1px solid #eee;"><b>Total Actual Physical Cash Counted (Drawers):</b></td>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: right;">{agg_actual_cash_counted:,.2f} TZS</td>
                </tr>
            </table>
        """
        ...
        if daily_summary_rows:
            range_report_html += """
            <h3 style="font-size: 15px; margin-bottom: 5px;">2. Daily Breakdown</h3>
            <table style="width: 100%; border-collapse: collapse; font-size: 12px; margin-bottom: 20px;">
                <thead>
                    <tr style="background-color: #f1f5f9; border-bottom: 2px solid #cbd5e1;">
                        <th style="padding: 6px; text-align: left;">Date</th>
                        <th style="padding: 6px; text-align: right;">Cash Sales</th>
                        <th style="padding: 6px; text-align: right;">Mobile Money</th>
                        <th style="padding: 6px; text-align: right;">Revenue</th>
                        <th style="padding: 6px; text-align: right;">Expenses</th>
                        <th style="padding: 6px; text-align: right;">Drawer Count</th>
                    </tr>
                </thead>
                <tbody>
            """
            for row in daily_summary_rows:
                range_report_html += f"""
                    <tr style="border-bottom: 1px solid #eee;">
                        ...
                        <td style="padding: 6px; text-align: right;">{row['Cash Sales']:,.2f}</td>
                        <td style="padding: 6px; text-align: right;">{row['Mobile Money']:,.2f}</td>
                        <td style="padding: 6px; text-align: right;">{row['Total Revenue']:,.2f}</td>
                        <td style="padding: 6px; text-align: right;">{row['Expenses Out']:,.2f}</td>
                        <td style="padding: 6px; text-align: right;">{row['Actual Cash Counted']:,.2f}</td>
                    </tr>
                """
            range_report_html += """
                </tbody>
            ...
            range_report_html += "<p style='color: #666; font-style: italic;'>No records found in memory for this date range. Please ensure records have been logged in the Daily Entry tab.</p>"

        range_report_html += """
            <hr style="border: 0; border-top: 1px solid #ccc; margin: 20px 0 10px 0;">
            <table style="width: 100%;">
                <tr>
                    <td><b>Prepared By:</b> _________________________</td>
                    <td style="text-align: right;"><b>Client / Manager Review:</b> _________________________</td>
                </tr>
            </table>
        </div>
        """
        ...
        st.components.v1.html(
            ...
            <div style="padding: 10px 0; text-align: center;">
                <button onclick="parent.window.print();" style="width: 100%; background-color: #1b365d; color: white; padding: 12px 20px; border: 1px solid #273e70; border-radius: 6px; font-weight: 500; letter-spacing: 0.5px; cursor: pointer; font-size: 15px; box-shadow: 0 2px 4px rgba(27, 54, 93, 0.2);">
                    🖨️ Print / Save Range Report as PDF
                </button>
            </div>
            """,
            height=70
...
# ----------------------------------------------------
# VIEW 3: MONTHLY REVIEW & ENTIRE MONTH PRINT REPORT
# ----------------------------------------------------
elif app_mode == "MONTHLY REVIEW":
    st.title("🗓️ Monthly Review & Full Month Statement")
    st.write("Select a specific month and year to view, aggregate, and print the complete daily log and monthly summary without losing any individual daily data.")
    ...
    with col_m2:
        ...
        selected_month_num = st.selectbox("Select Month", options=list(month_names.keys()), format_func=lambda x: month_names[x], index=datetime.now().month - 1)

    monthly_business_name = st.text_input("Business Name for Monthly Statement", "BUSINESS CLIENT")
    ...
    if st.button("📊 Generate Complete Monthly Report"):
        ...
        # Sort daily rows chronologically by date
        monthly_rows = sorted(monthly_rows, key=lambda x: x["Date"])
        total_monthly_revenue = m_cash_sales + m_mobile_money + m_card_sales + m_credit_sales
        ...
        <div id="printable-report" style="width: 100%; max-width: 750px; margin: 0 auto; padding: 25px; border: 1px solid #ccc; border-radius: 8px; background-color: #ffffff; color: #000; font-family: Arial, sans-serif; font-size: 13px;">
            <h2 style="text-align: center; margin-bottom: 2px; font-size: 22px;"><b>Cash Tracker</b><br><span style="font-size: 15px; font-weight: normal;">Daily sales</span></h2>
            <p style="text-align: center; font-size: 14px; color: #555; margin-top: 0;">{monthly_business_name} - Monthly Financial Statement</p>
            <p style="text-align: center; font-size: 12px; color: #777;">Month: {month_names[selected_month_num]} {selected_year}</p>
            <hr style="border: 0; border-top: 1px solid #ddd; margin: 15px 0;">
            
            <h3 style="font-size: 15px; margin-bottom: 5px;">1. Monthly Summary Totals</h3>
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 15px;">
                <tr>
                    <td style="padding: 6px; border-bottom: 1px solid #eee;"><b>Total Cash Collected:</b></td>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: right;">{m_cash_sales:,.2f} TZS</td>
                </tr>
                <tr>
                    <td style="padding: 6px; border-bottom: 1px solid #eee;"><b>Total Mobile Money Collected:</b></td>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: right;">{m_mobile_money:,.2f} TZS</td>
                </tr>
                <tr>
                    <td style="padding: 6px; border-bottom: 1px solid #eee;"><b>Total Card / Bank Sales:</b></td>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: right;">{m_card_sales:,.2f} TZS</td>
                </tr>
                <tr>
                    <td style="padding: 6px; border-bottom: 1px solid #eee;"><b>Total Credit Sales:</b></td>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: right;">{m_credit_sales:,.2f} TZS</td>
                </tr>
                <tr style="background-color: #f1f5f9;">
                    <td style="padding: 8px; font-weight: bold;">Combined Monthly Revenue:</td>
                    <td style="padding: 8px; font-weight: bold; text-align: right;">{total_monthly_revenue:,.2f} TZS</td>
                </tr>
                <tr>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; color: #b91c1c;"><b>Total Monthly Expenses / Cash Paid Out:</b></td>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: right; color: #b91c1c;">{m_expenses:,.2f} TZS</td>
                </tr>
                <tr>
                    <td style="padding: 6px; border-bottom: 1px solid #eee;"><b>Total Actual Cash Counted (Drawers):</b></td>
                    <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: right;">{m_actual_cash:,.2f} TZS</td>
                </tr>
            </table>
        """
        ...
        if monthly_rows:
            monthly_report_html += f"""
            <h3 style="font-size: 15px; margin-bottom: 5px;">2. Complete Daily Breakdown for {month_names[selected_month_num]} {selected_year}</h3>
            <table style="width: 100%; border-collapse: collapse; font-size: 11px; margin-bottom: 20px;">
                <thead>
                    <tr style="background-color: #f1f5f9; border-bottom: 2px solid #cbd5e1;">
                        <th style="padding: 5px; text-align: left;">Date</th>
                        <th style="padding: 5px; text-align: right;">Cash</th>
                        <th style="padding: 5px; text-align: right;">Mobile</th>
                        <th style="padding: 5px; text-align: right;">Card</th>
                        <th style="padding: 5px; text-align: right;">Credit</th>
                        <th style="padding: 5px; text-align: right;">Revenue</th>
            ...
            for row in monthly_rows:
                monthly_report_html += f"""
                    <tr style="border-bottom: 1px solid #eee;">
                        ...
                        <td style="padding: 5px; text-align: right;">{row['Cash Sales']:,.2f}</td>
                        <td style="padding: 5px; text-align: right;">{row['Mobile Money']:,.2f}</td>
                        <td style="padding: 5px; text-align: right;">{row['Card/Bank']:,.2f}</td>
                        <td style="padding: 5px; text-align: right;">{row['Credit']:,.2f}</td>
                        <td style="padding: 5px; text-align: right; font-weight: bold;">{row['Total Revenue']:,.2f}</td>
                        <td style="padding: 5px; text-align: right; color: #b91c1c;">{row['Expenses Out']:,.2f}</td>
                    </tr>
                """
            monthly_report_html += """
                </tbody>
            ...
            monthly_report_html += f"<p style='color: #666; font-style: italic;'>No daily logs found for {month_names[selected_month_num]} {selected_year}.</p>"
        ...
        st.components.v1.html(
            ...
            <div style="padding: 10px 0; text-align: center;">
                <button onclick="parent.window.print();" style="width: 100%; background-color: #1b365d; color: white; padding: 12px 20px; border: 1px solid #273e70; border-radius: 6px; font-weight: 500; letter-spacing: 0.5px; cursor: pointer; font-size: 15px; box-shadow: 0 2px 4px rgba(27, 54, 93, 0.2);">
                    🖨️ Print / Save Monthly Statement as PDF
                </button>
            </div>
            """,
            height=70
        ...
        if monthly_rows:
            ...
            st.download_button(
                ...
                file_name=f"monthly_statement_{month_names[selected_month_num]}_{selected_year}.csv",
                mime="text/csv",
            )
